...
#define CheckDevicePresence(iAddress) if (OW_ROMMatch(iAddress) == OW_NO_DEV) return DS1820_TEMP_ERROR

/* Internal functions */
static DS1820_State ScratchPadRead(uint8_t *bBuffer);
...
int DS1820_TemperatureGet(uint64_t iAddress) {
    ...
    if (ScratchPadRead(iSPad) == DS1820_CRC_ERROR) return DS1820_TEMP_ERROR;

    /* Calculate temperature from Scratchpad, step 1 */
    iTemp = iBinaryToIntTemperature(iSPad);
...
DS1820_State DS1820_TemperatureAlarmGet(uint64_t iAddress, int *iHigh, int *iLow) {
    ...
    /* Read DS1820 scratchpad, fail if CRC do not match */
    if (ScratchPadRead(iSPad) == DS1820_CRC_ERROR) return DS1820_ERROR;
 ...
 * @return DS1820_OK if CRC match, DS1820_CRC_ERROR if do not.
 */
DS1820_State ScratchPadRead(uint8_t *Buffer) {
    int i;
    uint8_t iCRC = 0;
    ...
    /* Match CRC */
    return (iCRC == Buffer[SCRATCHPAD_CRC_POS]) ? DS1820_OK : DS1820_CRC_ERROR;
}
