...
* Description    : Initializes LCD, TIM and FSMC
* Input          : None
* Output         : None
* Return         : None
* Attention	 : None
...
* Description    : Selects the register 0x22
* Input          : None
* Output         : None
* Return         : None
* Attention	 : None
...
/*******************************************************************************
* Function Name  : LCD_WriteRAM_Prepare
* Description    : Writes the RGB 565 code to RAM at the currebt cursor position
* Input          : RGB_Code: RGB 565 code
* Output         : None
* Return         : None
* Attention	 : None
*******************************************************************************/
...
* Description    : Writes the 16-bit value to the selected LCD register 
* Input          : LCD_Reg: reg index, LCD_RegValue: reg value
* Output         : None
* Return         : None
* Attention	 : None
*******************************************************************************/
...
* Description    : selects the LCD register
* Input          : - index: register index
* Output         : None
* Return         : None
* Attention	 : None
*******************************************************************************/
static inline void LCD_WriteIndex(uint8_t LCD_Reg)
{
	LCD_REG	= LCD_Reg;
}
...
* Description    : Reads the 16-bit value from the LCD
* Input          : None
* Output         : None
* Return         : the 16-bit value from the LCD 
* Attention	 : None
*******************************************************************************/
...
* Input          : LCD_reg: register to be read
* Output         : None
...
void LCD_PutChar(int16_t PosX, int16_t PosY, uint8_t c)
{
  uint8_t i = 0;
  uint8_t j = 0;
  if(asciisize==8)
...
void LCDprint( char *ptr ) {
	...
	// print characters except the NULL char on the LCD:
	while ((*ptr) != '\n') {
		LCDputc(*ptr);
		ptr++;
	}
...
void LCD_DrawRect(uint16_t Xpos, uint16_t Ypos, uint8_t Width, uint16_t Height)
{
 int x,y;
 x=0;
 y=0;
 while(x<Width+1)
 {
   ...
   PutPixel(Xpos+x,Ypos+Height);
   x++;
 }
 while(y<Height+1)
 {
   PutPixel(Xpos,Ypos+y);
   PutPixel(Xpos+Width,Ypos+y);
   y++;
 }
}
...
void LCD_FSMCConfig(void)
{
  ...
  /* FSMC Write Timing */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 5;//2;//1;   /* Address Setup Time */
  FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
  FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 5;//2;//1;	     /* Data Setup Time */
  FSMC_NORSRAMTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_CLKDivision = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_DataLatency = 0x00;
  FSMC_NORSRAMTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_A;	/* FSMC Access Mode */
...
/* Pa3cio, 11.1.2013*/
void LCD_SetDisplayWindow(uint8_t Xpos, uint16_t Ypos, uint8_t Width, uint16_t Height)
{
  volatile uint16_t reg44value;
  
  
  reg44value = 0x0000;
  reg44value = reg44value | (Width + Xpos - 1);
  reg44value = reg44value << 8;
  reg44value = reg44value | Xpos;
  
  /* Set HSA and HEA of the window, PB 11.1.2013 */
  LCD_WriteReg(0x0044, reg44value);
  Delay(50);
  /* Set VSA of the window, PB,11.1.2013 */
  LCD_WriteReg(0x0045, Ypos);
  Delay(50);
  /* Set VSA of the window, PB,11.1.2013 */
  LCD_WriteReg(0x0046, (Height + Ypos - 1));
  Delay(50);
...
Pa3cio Bulic, 14.1.2013

*/
void LCD_ClearRectangle(uint8_t Xpos, uint16_t Ypos, uint16_t Height, uint8_t Width, uint16_t color)
{
  ...
  /*
  Display window should be of the same size as bitmap:
  */
  LCD_SetDisplayWindow(Xpos, Ypos, Width, Height);
  
  /* 
  Set type A color coding, left2right and up2bottom scan
  Pa3cio, 11.1.2013
  */
...
Pa3cio Bulic, 11.1.2013


Images can be converted to array with:
http://www.meraman.com/htmls/en/ImageConvert.html

bitmap shoud be in 16 bpp, RGB565, linear array of type uint16_t

*/
void LCD_WriteBMP(uint8_t Xpos, uint16_t Ypos, uint16_t Height, uint8_t Width, uint16_t *bitmap)
{
  volatile uint32_t index;
